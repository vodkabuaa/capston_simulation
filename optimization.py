...
class GradientSearch(object) :
	...
	def _accelerate_scale(self, accelerator, history_grad, grad):
		...
		scale_vector = np.ones(self.var_nums) * ( 1 + 0.025)
		accelerator[sign_vector <= 0] = 1
		return accelerator * scale_vector

	def _adam(self, t, history_grad, grad_t, window):
		"""
		...
		ita = 0.001
		eps = 1e-8
		m_t = np.mean(history_grad[-window:], axis = 0)
		v_t = np.mean(np.power(history_grad[-window:], 2), axis =0)
	...
	def gradient_descent(self, initial_point, window=30, return_last=False):
		...
		return x_hist[-10:][best_index], u_hist[-10:][best_index]
